		...
		<div class="row">
			<div class="col d-flex justify-content-start align-items-center">
				...
				<div class="title ml-4" >
					<h2 class="text-white text-uppercase mb-0">Homepage</h2>
				</div>
			</div> <!-- end col -->
							...
							<div class="row">
								<div class="form-group lpnu-form col-md-3">
									...
									<div class="input-group input-group-merge">
										...
										<input type="text" name="halaman" value="homepage" class="form-control text-center" readonly="readonly">
									</div>
								</div>
...
<script type="text/javascript">
	jQuery(document).ready(function($) {
		...
		<?php 
			if (session()->getFlashdata('admin_tataletak_homepage_msg')) {
				echo "showAlert('" . session()->getFlashdata('admin_tataletak_homepage_msg') . "', 'fas fa-thumbs-up', 'default')";
				echo "\n";
			}
		?>
		...
		// * -------------------------------------------------------------
		// * Load Content - halaman list layout
		// --------------
		function refreshContent(affected_id) {
			$.ajax({
				url: '<?=site_url('admin/tataletak/ajax_list/homepage_content')?>',
				type: 'GET',
				dataType: 'html',
				data: {
					halaman: 'homepage',
			...
			.done(function(data) {
				$(".load-content").html(data).promise().done(function() {
					onContentLoad()
		...
		function onContentLoad() {
			...
			$(".btn-tambah-element-col").click(function(e) {
				row = $(this).data('row')
				maxCol = $(this).data('max-col')
				$("#modal-tambah-elemen").modal('show')
				$("#modal-tambah-elemen [name='halaman']").val('homepage')
				$("#modal-tambah-elemen [name='penempatan']").val('content')
				$("#modal-tambah-elemen [name='row']").val(row)
				$("#modal-tambah-elemen [name='urutan_col']").val(parseInt(maxCol + 1))
				e.preventDefault()
			})
			$(".btn-tambah-element-row").click(function(e) {
			...
			$(".btn-hapus").unbind().click(function(e) {
			...
			$(".btn-move-row-up").unbind().click(function(e) {
				e.preventDefault()
				row = $(this).data('row')
				...
				.done(function(data) {
					if (data.status == 'success') {
						refreshContent(data.affected_id)
						showAlert(data.text, data.icon, data.color)
					}
				})
			})
			$(".btn-move-row-down").unbind().click(function(e) {
				e.preventDefault()
				row = $(this).data('row')
				penempatan = $(this).data('penempatan')
				halaman = $(this).data('halaman')
			...
			$(".btn-move-col-left").unbind().click(function(e) {
				e.preventDefault()
				id = $(this).data('id')
				$.ajax({
					url: '<?=site_url('admin/tataletak/ajax_write/moveup')?>',
			...
			$(".btn-move-col-right").unbind().click(function(e) {
				e.preventDefault()
				id = $(this).data('id')
				$.ajax({
					url: '<?=site_url('admin/tataletak/ajax_write/movedown')?>',
		...
		$("#modal-tambah-elemen").on('shown.bs.modal', function(e) {
			...
			modal.find("form").unbind().submit(function(e) {
				...
				.done(function(data) {
					if (data.status == 'success') {
						refreshContent(data.inserted_id)
