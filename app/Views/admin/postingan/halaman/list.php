...
<script type="text/javascript">
	jQuery(document).ready(function($) {
		...
		function refreshList() {
			...
			$.ajax({
				url: '<?=site_url('admin/postingan/halaman/ajax_list/list-row-main')?>',
				type: 'GET',
				dataType: 'html',
				data: params,
			})
		...
		function onLoadList() {
			...
			$(".btn-content-hapus").unbind('click').click(function(e) {
				e.preventDefault();
				if (confirm('Hapus Halaman, Anda Yakin?')) {
					if (confirm('halaman yang terhapus tidak dapat dikembalikan lagi, Anda yakin?')) {
						id = $(this).data('id')
						$.ajax({
							url: '<?=site_url('admin/postingan/halaman/ajax_delete')?>',
							type: 'POST',
							dataType: 'json',
							data: {id: id},
						})
