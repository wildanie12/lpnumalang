...
<?php 
	...
	$description = "";
	if ($artikel['deskripsi_penelusuran'] != '') 
		$description = $artikel['deskripsi_penelusuran'];
	else 
	...
	$created_at = new DateTime($artikel['created_at']);
	$updated_at = new DateTime($artikel['updated_at']);
...
<meta property="twitter:url" content="<?=site_url('/' . $artikel['slug'])?>">
		...
		<div class="card">
			<div class="card-body">
				...
				<div class="row article">
					<div class="col">
						<h1 class="mt-0 mt-2 mb-2" style="font-size: 20px;font-family: 'Raleway', sans-serif;font-weight: 600;"><?=$artikel['judul']?></h1>
						<div class="d-flex mb-3 flex-row justify-content-between align-items-start">
							...
							<div>
								<a target="_blank" href="http://www.facebook.com/sharer.php?u=<?=site_url($artikel['slug'])?>" class="btn btn-sm btn-link text-white p-1" style="background-color: #3b5998; width: 30px; height: 30px">
									<i class="fab fa-facebook-f" style="font-size:15pt; top: 1px; position: relative;"></i>
								</a>
								<a target="_blank" href="http://twitter.com/share?url=<?=site_url($artikel['slug'])?>" class="btn btn-sm btn-link text-white p-1" style="background-color: #55acee; width: 30px; height: 30px">
									<i class="fab fa-twitter" style="font-size:15pt; top: 1px; position: relative;"></i>
								</a>
								<a target="_blank" href="mailto:?Subject=Artikel Berjudul <?=$artikel['judul']?>&Body=Kunjungi selengakpnya di <?=site_url($artikel['slug'])?>" class="btn btn-sm btn-link text-white p-1" style="background-color: #D93025; width: 30px; height: 30px">
									<i class="fas fa-envelope" style="font-size:15pt; left: -1px; position: relative;"></i>
								</a>
								<a target="_blank" href="whatsapp://send?text=Artikel berjudul <?=$artikel['judul']?>, Kunjungi selengakpnya di <?=site_url($artikel['slug'])?>" data-action="share/whatsapp/share" class="btn btn-sm btn-link text-white p-1" style="background-color: #28A219; width: 30px; height: 30px">
									<i class="fab fa-whatsapp" style="font-size:15pt; top: 0px; position: relative;"></i>
								</a>
								<a target="_blank" href="http://www.linkedin.com/shareArticle?mini=true&url=<?=site_url($artikel['slug'])?>" class="btn btn-sm btn-link text-white p-1" style="background-color: #007bb5; width: 30px; height: 30px">
			...
			<div class="card-body">
				<div class="row">
					<div class="col d-flex justify-content-between align-items-center">
						<h4 style="font-weight: bold; font-size: 12pt;" class="m-2">Artikel Terkait</h4>
					</div>
				</div>
				<?php 
					$kategori_array = explode('|', $artikel['kategori_id']);
					foreach ($kategori_array as $kategori_id) 
						$artikelModel->like('kategori_id', $kategori_id, 'both');
					$data_artikel = $artikelModel->findAll(6);
